...
static bench_result run_benchmark(common_params & params, const bench_config & cfg, int run_idx) {
    ...
    if (common_speculative_has_det_filter(spec)) {
        ...
        if (plugin) {
            ...
            // Grammar selection is handled entirely by the plugin's bootstrap
            // language auto-detection (it narrows bundled grammars via XGrammar
            // accept/reject as tokens are generated). No explicit call needed.
        }
    }
