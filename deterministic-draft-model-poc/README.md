...
| `--lang LANGUAGE` | Select grammar by language name (benchmark tool). Resolves from `grammars/<lang>.gbnf` bundled alongside the `.so`. |

| Environment Variable | Description |
|---|---|
| `DETERMINISTIC_DRAFT_LANGUAGE` | Language name for grammar selection (alternative to `--lang`, useful for programmatic use). If set, the plugin auto-loads the corresponding bundled grammar during initialization. |
