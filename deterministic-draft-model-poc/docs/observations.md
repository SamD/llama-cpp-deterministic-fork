...
This session's changes have no effect on output distribution or perplexity.
The target model verifies every accepted token via standard rejection sampling,
which mathematically guarantees the output distribution matches the target
model's true distribution regardless of draft/grammar-filter changes. The
changes in this session (per-slot grammar isolation, bootstrap language
auto-detection, the fail-open-to-fail-closed termination fix, c.gbnf grammar
corrections, per-token allocation removal, benchmark loop fixes, --lang/env
var removal) affect only draft acceptance rate and generation speed, never the
...
`#include` line (`#include <stdio.h> int main(void) {`) leaves the
c.gbnf matcher stuck inside its `preprocessor` rule (`[^\n]* "\n"`), so
it accepts any non-newline token (including non-C text) instead of
validating C. Use a newline-terminated prompt
...
- **python.gbnf assignment-target allowed the call trailer**: `target ::
  ...
  error - a call is not a valid target). Treatment scored 0/5 valid
...
- **Removed `DETERMINISTIC_DRAFT_LANGUAGE` env var and `--lang` CLI flag**:
  both were removed entirely. Grammar selection is now driven solely by the
  plugin's bootstrap language auto-detection (no external language override
  remains).
