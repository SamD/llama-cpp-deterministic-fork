...
**Status: XGrammar-based (bitmask-constrained decoding).** The plugin utilizes XGrammar's token-level bitmask constraint (`fill_bitmask`/`commit`), rather than post-hoc text validation. The Phase 2 tables below reflect the current numbers; the historical tree-sitter-era results have been purged (though recoverable from git history). Re-run section 3.7's automation script against your own model/hardware for current results. For multi-hardware comparison (RTX 4070, GTi15 Arc PRO B70, N100) see [benchmark-overview.md](benchmark-overview.md).

---

## Phase 2 (2026-07-18, RTX 3060, Qwen3.5-9B) - current XGrammar numbers

These are the latest results, achieved with the current XGrammar plugin after the Phase 2 correctness/performance fixes (prompt ingestion into the draft context, newline-terminated prompt, O(1) `AcceptToken` bonus verification replacing `FillNextTokenBitmask`, empty-draft grammar constraint, double-commit fix - see [observations.md](observations.md)). Unlike the Phase 1 / tree-sitter-era runs, accept-all here produces **gcc-valid** output and is faster than baseline.
...
1. **The filter guarantees validity the raw model does not** - baseline produced invalid C in 3/10 runs; the filter was valid in 10/10. Baseline output is now gcc-validated in compare mode (previously unchecked), and the `caught by filter` row counts runs where the raw model broke but the filter held.
...
### C (4 benchmark files + 2 edge-case files)
...
and `06_valid_if_else.c.test`, used by `test_file_based.py` rather than the
benchmark matrix.
...
> **Keep the prompt's real newlines (Phase 2 requirement).** The grammar matcher is primed with the prompt, so it must be valid input for the grammar. Do NOT flatten the prompt to one line - for C, `#include <stdio.h>` must keep its trailing newline, or the c.gbnf matcher stays stuck in its `preprocessor` rule and accepts anything instead of validating. See the note in quick-start.md and the automation-script warning in section 3.7.
...
The language is auto-detected by the plugin's bootstrap detection from the generated content - there is no `--lang` flag. Give the model a prompt that is unambiguously in the language you want (e.g. a C snippet) so detection resolves to it. The detected language is reported in the output/logs.
...
for entry in "${TESTS[@]}"; do
  ...
  for n_max in "${N_MAX_VALUES[@]}"; do
    ...
    # c.gbnf matcher stays stuck in its `preprocessor` rule and accepts anything
    # instead of validating. Pass "$prompt" (newlines preserved), not a flattened
    # variant. The historical version of this script flattened the prompt; that
    # breaks grammar-constrained runs.
    prompt_flat="$prompt"
