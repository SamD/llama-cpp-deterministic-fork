...
void common_speculative_accept(common_speculative * spec, llama_seq_id seq_id, uint16_t n_accepted, llama_token bonus_token) {
    if (!spec) {
        return;
    }
    ...
    if (spec->det_filter.plugin) {
        // Draft tokens were already committed to the grammar during
        // common_speculative_draft()'s bitmask-filtering step. In accept-all
        // mode the grammar is the sole verifier, so n_accepted always matches
        // the filtered draft size and nothing further is needed here.
        //
        // If det_filter is combined with standard target-model rejection
        // sampling, the target may accept FEWER tokens than the grammar
        // already committed - roll the grammar back to stay consistent with
        // what was actually emitted.
        const auto & fr = spec->det_filter.last_result[seq_id];
        const int    n_committed = fr.valid_count;

        if (!common_speculative_get_det_accept_all(spec) && n_accepted < (uint16_t) n_committed) {
            if (!llama_deterministic_draft_rollback(
                    spec->det_filter.plugin, seq_id, n_committed - (int) n_accepted)) {
                LOG_ERR("%s: failed to rollback grammar state for seq_id %d, resetting slot\n", __func__, seq_id);
                llama_deterministic_draft_reset(spec->det_filter.plugin, seq_id);
            }
        }

        // In accept-all mode the bonus token was already committed to the
        // grammar by filter_draft during common_speculative_sample_and_accept()'s
        // bonus selection, so committing it here would double-commit.
        if (bonus_token != LLAMA_TOKEN_NULL &&
            !common_speculative_get_det_accept_all(spec)) {
            llama_deterministic_draft_commit_tokens(
                spec->det_filter.plugin, seq_id,
                &bonus_token, 1);
        }
...
static llama_tokens common_speculative_sample_and_accept_all(
    ...
    if (spec->det_filter.plugin) {
        // Grammar already reached a complete parse: there is no valid
        // bonus token, and this is a legitimate end state, not an error.
        // Return the draft without resetting the matcher (resetting here
        // would desync it from the actual generated content).
        if (common_speculative_is_terminated(spec, seq_id)) {
            return result;
        }

        // Fast bonus selection: probe the sampler's probability-sorted
        // shortlist with O(1) AcceptToken checks (filter_draft commits on
        // accept) and take the highest-probability grammar-valid token.
        // This avoids the O(vocab) FillNextTokenBitmask call, which costs
        // ~1s+ per step with the C grammar. The accepted token is committed
        // to the grammar by filter_draft here, so common_speculative_accept()
        // must not commit it again (see the accept-all guard there).
        llama_token bonus = LLAMA_TOKEN_NULL;
        {
            common_sampler_sample(smpl, ctx, idxs.back());
            const auto * cur_p = common_sampler_get_candidates(smpl, true);
        ...
        if (bonus == LLAMA_TOKEN_NULL) {
            ...
            float * logits = llama_get_logits_ith(ctx, idxs.back());
            ...
            if (!llama_deterministic_draft_apply_bitmask(
                ...
                LOG_ERR("%s: failed to apply bitmask for bonus token, seq_id %d, skipping bonus\n", __func__, seq_id);
                return result;
            }
            bonus = common_sampler_sample(smpl, ctx, idxs.back());
            // bitmask already guarantees validity, so filter_draft accepts
            // and commits the token here (same accounting as the fast path).
            llama_deterministic_draft_filter_draft(
                    spec->det_filter.plugin, seq_id, &bonus, 1);
        }
